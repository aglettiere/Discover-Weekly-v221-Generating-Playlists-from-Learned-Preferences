...
def makeGenreDictTree(filename,genreArray, masterGenres, song) : 
    with open(filename) as f: 
        line = csv.reader(f)
        rowNum = 0 
        for row in line: 
            ...
            for value in row : 
                ...
                currFeature+=1 
            # genreDict[nameID] = str(gname) 
            if(gname in masterGenres and nameID!=song):
                genreArray.append(gname) 
            rowNum += 1 
def readMusicDataTree(filename, masterGenres, song) :
    # open a file
    featMatrix = []
    with open(filename) as f:
        line = csv.reader(f)
...
with open('SpotifyFeatures.csv','r') as csvinput:
    with open('singlePrediction.csv', 'w') as csvoutput:

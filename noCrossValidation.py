...
def outRes2(x, songIds, toUse = [], average = True) : 
    x, fbhjfh = numericalFeaturesOnly(allFeaturesMatrix, desFeat2) 
    necessaryDict = {} 
    totalSongs = 0 
    numIncorrect = 0  
#     print("x0::::::")
...
#         print(genreDict[songIds[i]])
        ...
        songIdToPredictedGenre[songIds[i]] = scoresDict[maxScore]
        if(scoresDict[maxScore]!=genreDict[songIds[i]]): 
...
#     print(genreArray) 
    # print(X_train[1])
    # numericalFeatures, dfbnhjf = numericalFeaturesOnly(allFeaturesMatrix, desFeat2) 
...
# print(len(y_test))
numInc, tot = outRes2(allFeaturesMatrix, songIds, totalSongs)
incorrect += numInc
totalSongs+= tot
print("num incorrect")
print(incorrect)
...
print(totalSongs)
print("accruacy")
print(float(incorrect)/float(totalSongs))


with open('SpotifyFeatures.csv','r') as csvinput:
    with open('singlePredictionTRAINALL.csv', 'w') as csvoutput:
